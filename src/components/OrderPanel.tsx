 ...
 */
/**
 * Квитанция: что с заказом, который человек уже начал оплачивать.
 *
 * Нужна ровно одному человеку — тому, кто ушёл платить и вернулся
 * на сайт. Для него это единственная дорога к своим деньгам: входа
 * на сайте нет, и опознать его нечем. Всем остальным её не видно.
 *
 * ДВА СОСТОЯНИЯ, И РАЗНИЦА МЕЖДУ НИМИ — ГЛАВНОЕ ЗДЕСЬ. Пока денег
 * нет, это напоминание: «вы начали оплату». Как только бот говорит,
 * что заказ оплачен, — это уже другое сообщение, и выглядит оно
 * по-другому: человек, отдавший деньги, обязан увидеть, что они
 * дошли, не перезагружая страницу руками. Кто спрашивает бота
 * и когда — написано в `order.tsx`.
 *
 * НОМЕРА ЗАКАЗА ЗДЕСЬ НЕТ — то же решение владельца, что и в боте:
 * везде, где его видит покупатель, стоит «ваш заказ». Номер остаётся
 * помощнику, ему он нужен для привязки кода.
 */
function Kvitanciya() {
  const { kvitanciya, zabytKvitanciyu } = useOrder();
  if (!kvitanciya) return null;
  const oplachen = Boolean(kvitanciya.oplachen);
  return (
    <div className={`kvit${oplachen ? ' kvit--oplachen' : ''}`} role="status">
      <p className="kvit__text">
        {oplachen
          ? 'Заказ оплачен. Заберите его в боте — там спросят, новый аккаунт или свой.'
          : 'Вы начали оплату заказа.'}
      </p>
      <a className="kvit__cta" href={kvitanciya.vBot} target="_blank" rel="noopener noreferrer">
        {oplachen ? 'Забрать заказ в боте' : 'Открыть заказ в боте'}
      </a>
      ...
      <div className="order__paper">
        ...
        <div className="order__foot">
          ...
          <p className="order__fineprint">
            ...
            {botReady
              ? 'Оплачиваете выбранный тариф на сайте. Логин и пароль приходят в Telegram. В рабочее время (10:00–22:00 МСК) это занимает 5–30 минут. Ночные заказы выполняются с утра в первую очередь. Если что-то не работает — пишите в чат поддержки в Telegram, решим проблему или вернём деньги.'

 ...
 * Добегающий счётчик.
 *
 * У УЗЛА СО СЧЁТЧИКОМ НЕ ДОЛЖНО БЫТЬ ДРУГИХ ДЕТЕЙ, и это не
 * придирка. Хук пишет в него `textContent`, то есть сносит всех
 * детей разом; React о сносе не знает и при следующей отрисовке
 * падает на `removeChild` — «The node to be removed is not a child
 * of this node», — унося с собой весь поддеревом блок. Так уже
 * случилось с зачёркнутой ценой рядом с итогом на нижней полосе:
 * чек пропадал целиком после снятия промокода. Соседей ставить
 * можно и нужно, детей — нельзя.
 */
export function useCountUp<T extends HTMLElement = HTMLSpanElement>(
  ...
  useIsomorphicLayoutEffect(() => {
    ...
    const tween = gsap.to(proxy, {
      ...
      onComplete: () => {
        /* А ПРИХОДИТ — В ТОЧНОЕ ЗНАЧЕНИЕ. Пока здесь стояла одна
           запись `shown.current`, на экране навсегда оставался
           последний кадр разбега, то есть округлённое до рубля
           число: 1 399 ₽ со скидкой 10 % показывались как 1 259 ₽
           вместо 1 259,10 ₽. Десять копеек — мелочь, но это чек,
           и он обязан сходиться с тем, что человек заплатит. */
        node.textContent = format(value);
        shown.current = value;
      },
    });

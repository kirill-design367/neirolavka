...
export function mount(root: HTMLElement, canvas: HTMLCanvasElement, cards: HTMLElement[]): Shelf | null {
  ...
  const targetOf = (i: number) => {
    ...
    // Дальняя карточка СПУСКАЕТСЯ, и без этого её не видно вовсе.
    // Вбок ей идти уже некуда — колонка кончилась, — а по ширине
    // и высоте она целиком помещается в коробку предыдущей: замер
    // показывал перекрытие 100 %, то есть продукт, который нельзя
    // ни увидеть, ни нажать. Спуск на 26 px выводит её нижний край
    // из-за соседа, и колода читается колодой.
    const SPUSK = [-10, 0, 10, 30];
    const tier = Math.min(d, 3);
    ...
    if (p === 0) {
      return giveWay({ x: 0, y: SPUSK[0], z: GLUBINA[0], ry: 0, s: MASHTAB[0], o: 1, lift: 1 });
    }

    const side = {
      x: dir * DOLYA_X[tier] * spread,
      y: SPUSK[tier],
      z: GLUBINA[tier],
      ry: -dir * POVOROT[tier],
      s: MASHTAB[tier],

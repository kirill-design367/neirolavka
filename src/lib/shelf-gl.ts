...
export function mount(root: HTMLElement, canvas: HTMLCanvasElement, cards: HTMLElement[]): Shelf | null {
  ...
  const targetOf = (i: number) => {
    ...
    //
    // У чётного числа карточек противоположная уходит на одну
    // из сторон — здесь вправо. Симметрии тут быть не может
    // по построению: пять соседей на две стороны не делятся.

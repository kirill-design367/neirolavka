...
const FRAG = /* glsl */ `
  ...
  const vec3 SVET = vec3(-0.45906, 0.54888, 0.69857);
  // Половинный вектор между светом и взглядом (0,0,1) — тоже
  // константа. Оба записаны СРАЗУ нормированными: считать
  // normalize на каждый пиксель незачем.
  const vec3 POLU = vec3(-0.24907, 0.29777, 0.92162);
  ...
  void main() {
    ...
    // Тридцать вторая степень берётся ПЯТЬЮ УМНОЖЕНИЯМИ, а не pow.
    // pow — это exp2(степень * log2(x)), две трансцендентные функции
    // на каждый пиксель; на поле такой плотности и на машине без
    // ускорителя это видно по кадрам, а на глаз 32-я степень от 34-й
    // не отличается ничем.
    float b1 = max(dot(n, POLU), 0.0);
    float b2 = b1 * b1;
    float b4 = b2 * b2;
    float b8 = b4 * b4;
    float sp = b8 * b8 * b8 * b8;
    ...
    // Куб — тоже умножениями, по той же причине.
    float t = 1.0 - abs(n.z);
    float rim = t * t * t * smoothstep(0.62, 0.05, lit) * 0.5;
...
export function mount(canvas: HTMLCanvasElement, host: HTMLElement): (() => void) | null {
  ...
  // Число пузырей ВДВОЕ МЕНЬШЕ прежнего: 57 → 28 на десктопе,
  // 14 → 7 на телефоне. Так решил владелец — поле забирало на себя
  // слишком много внимания.
  //
  // Тронуто ТОЛЬКО количество. Размер точки, их число на пузырь
  // (0.26·r²), плотность краски, границы радиусов и лесенка разброса
  // остались прежними: лесенка делит тот же отрезок 15–54 px
  // на вдвое меньше ступеней, то есть калибры стали реже, а размах
  // остался тем же.
  //
  // Прежнее число бралось как плотность на экране, помноженная
  // на площадь холста: холст занимал 900×504 на десктопе и 366×733
  // на телефоне, потом стал во всё окно — площадь выросла втрое
  // и в 1.23 раза, отсюда и были 19 → 57 и 11 → 14.
  //
  ...
  const HOTIM = phone ? 7 : 28;
  // Потолок берём у самой машины, а не на веру: на устройстве
  // с гарантированным минимумом векторов пузыри просто не собрались
  // бы, и шейдер не слинковался бы вовсе. Восемь векторов оставлены

...
export function mount(canvas: HTMLCanvasElement, host: HTMLElement): (() => void) | null {
  ...
  const tochkaU = (r: number) => {
    ...
    // Размер поднят в 1.8 раза, и это ЕДИНСТВЕННОЕ, что вернуло точке
    // круглую форму.
    //
    // Замер округлости (радиус по диагоналям к радиусу по осям; 1.00 —
    // круг, 1.41 — квадрат), медиана по сотне одиночных пятен на живой
    // странице, экран обычной плотности:
    //
    //   точка 3.00 css-px .................. 0.954, худшая 0.771
    //   она же, нарисованная квадами ....... 0.968, худшая 0.800
    //   точка 3.78 css-px .................. 0.976, худшая 0.902
    //   точка 5.43 css-px .................. 0.992, худшая 0.923
    //
    // То есть примитив ни при чём: переписывание точки с gl.POINTS
    // на инстансированные квады с аналитической кромкой дало +0.014
    // округлости и 100 % кадров дольше 17 мс вместо 0.6 %. А размер
    // решает: у точки в три пикселя круга не бывает ни при каком
    // способе отрисовки — столько пикселей просто нечем занять.
    return (phone ? 3.8 : 4.3) + (phone ? 3.6 : 5.0) * d;
  };
  ...
  const resize = () => {
    ...
    // Плотность холста — ПОЛНОЕ РАЗРЕШЕНИЕ ЭКРАНА, один к одному.
    //
    // Здесь стояло `Math.min(1, devicePixelRatio)`, и это оказалось
    // причиной рваных краёв: на дисплее с удвоенной плотностью холст
    // рисовался вдвое мельче экрана и растягивался обратно. Точка
    // шириной три css-пикселя получала три отсчёта буфера вместо
    // шести, и её мягкий круглый край превращался в ступеньку.
    //
    // Понижать плотность обратно нельзя ни бюджетом, ни потолком:
    // любое понижение возвращает ту же ступеньку.

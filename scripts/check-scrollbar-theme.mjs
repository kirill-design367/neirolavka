...
// Холст пузырей убирается на ВСЁ время съёмки, а не между кадрами.
//
// Он закреплён по окну, лежит за всем содержимым и не ловит мышь —
// значит elementFromPoint его не видит вовсе, и точка, по которой
// ползёт пузырь, объявляется чистым фоном страницы. Дальше замер
// требует совпадения цвета до канала, а пузырь за 900 мс перехода
// уводит свою точку на десятки уровней. Замерено: из 204 точек,
// сходящих здесь за чистый фон, под пузырями оказывается от 3 до 14 —
// от загрузки к загрузке по-разному, размах до 152 уровней при
// допуске в один. Попади выбор на такую точку, и полоса с фоном
// разойдутся на ровном месте, причём не каждый раз: пузыри при каждой
// загрузке встают в новых местах, и проверка мигала бы без причины.

...
for (const SCALE of SCALES) {
  ...
  // Цвет подменяется на РАВНЫЙ ПО СВЕТЛОТЕ (0.0863 против 0.0865
  // у #365c30), просто другого тона. Пара разной светлоты
  // обесценивает контроль: покрытие краской считается порогом,
  // и бледный цвет на бежевом даёт меньше «краски» сам по себе.
  const ctlB = await shotColored('light', ['#7c3e63', '#7c3e63']);

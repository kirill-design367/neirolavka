...
for (const theme of ['light', 'dark']) {
  ...
  const GROUPS = [
    // Карточек условий здесь БОЛЬШЕ НЕТ, и это не забывчивость.
    // Проба берёт самый светлый пиксель подложки за 12 фаз цикла —
    // она про ПЕРЕЛИВ. С карточек условий перелив снят (они стали
    // ровным деревом), и все двенадцать фаз у них теперь одинаковы:
    // проба осталась бы вечнозелёной и перестала бы что-либо
    // проверять — тот самый случай «проверка, которая перестала
    // проверять, опаснее упавшей». Те же пары полностью закрывает
    // check-contrast.mjs, который обходит страницу целиком
    // и меряет их по настоящим пикселям (5.09:1 и 5.74:1).

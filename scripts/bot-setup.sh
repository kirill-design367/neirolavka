...
#
# Файл читает systemd (`EnvironmentFile=`), а не bash: у него свой
# разбор, строка `ИМЯ=значение`, никакого выполнения. Строка, которая
# под это не подходит, для systemd — мусор, а для всякого, кто
# попробует подключить файл оболочкой, — КОМАНДА: bash выполнит её
# и напечатает обратно вместе с содержимым. Если такой строкой
# окажется съехавший при вставке токен, он окажется на экране.
#
# Поэтому формат проверяется здесь, а НОМЕРА негодных строк
# печатаются без содержимого: печатать содержимое значило бы
# повторить ровно ту утечку, от которой проверка защищает.
# ─────────────────────────────────────────────────────────────────────
PLOHIE="$(awk '
  { sub(/\r$/, "") }
  /^[[:space:]]*$/ { next }
...
' "$OKR")"
if [ -n "$PLOHIE" ]; then
  vni "в $OKR не читаются строки: $PLOHIE"
  vni "нужно «ИМЯ=значение» одной строкой: без пробелов вокруг «=», без export,"
  vni "и значение НЕ переносится на следующую строку. Содержимое не печатаю —"
  vni "именно в такой строке обычно и лежит секрет."
  vni "Откройте эти строки в редакторе: sudo nano +$(printf '%s' "$PLOHIE" | cut -d, -f1) $OKR"
  vni "Чаще всего это одно значение, разорванное переносом, — сшейте его в одну строку."
else
  ok "формат окружения в порядке: все строки читаются как ИМЯ=значение"
fi

 ...
 *
 *    Чего проверка НЕ ловит, и это надо назвать прямо: форму кривой.
 *    Линейное движение — такое же одно движение, просто с другим
 *    ощущением; провал у него 0.2–7.8 %, то есть ниже порога, и это
 *    верно. За форму кривой отвечают --ease-shelf и --ease-fold
 *    с пояснением, откуда взяты, а не эта проверка.
 *
 * Парение карточек на время замера выключается: иначе меряется оно.
 *
 * Запуск: node scripts/check-shelf-motion.mjs http://localhost:4173/
 */

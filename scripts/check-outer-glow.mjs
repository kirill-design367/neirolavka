...
// Пузыри с недавних пор живут не внутри первого экрана, а холстом,
// закреплённым по окну за всем содержимым страницы, и не замирают
// ни на миг. Собственного фона у `.referral` нет, поэтому они видны
// и сквозь блок, и вокруг него — ровно там, где меряется свет.
// Проверка вычитает два снимка друг из друга, а между снимками
// пузыри проезжают: их дрейф попал бы в «прирост яркости» целиком
// и был бы засчитан свету. До этой правки он давал до 161 уровня
// в столбце наибольшего прироста на расстояниях, куда подсветка
// не долетает вовсе.
// Прячем на ВСЁ время замера, а не между снимками: холст лежит вне
// потока, и от его исчезновения в композиции не сдвигается ничего.
await p.addStyleTag({ content: 'canvas.bubbles{display:none!important}' });

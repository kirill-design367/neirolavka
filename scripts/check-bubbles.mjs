...
/** Доля различий двух снимков одного места. */
const raznica = (a, b) => {
  let s = 0;
...
for (const r of RAZRESHENIYA) {
  ...
  if (!kuchi.length) {
    ...
    // ── отклик на курсор ──
    //
    // Сравнение КАРТИНКИ С КАРТИНКОЙ в коробке вокруг пузыря.
    // Пузыри плывут, поэтому два соседних снимка отличаются и сами
    // по себе — это шум метода, он меряется тут же, при убранном
    // курсоре и за то же время, а вердикт ставится на отношении.
    //
    // Размах облака вокруг найденной середины пробовался и отвергнут:
    // он требует, чтобы курсор стоял ТОЧНО в середине, иначе сжатие
    // вдоль оси съедает рост, и на исправной сборке выходило от −6
    // до +29 %. Середину же взять неоткуда: решётка видит только
    // ту часть пузыря, что не закрыта содержимым. Разница картинок
    // этого не требует — деформация меняет картинку, где бы курсор
    // ни стоял.
    const kor = {
      x: Math.max(0, cx - rr - 22),
      y: Math.max(0, cy - rr - 22),
      width: Math.min(r.w, cx + rr + 22) - Math.max(0, cx - rr - 22),
      height: Math.min(r.h, cy + rr + 22) - Math.max(0, cy - rr - 22),
    };
    // Курсор уводится ВЫШЕ первого экрана: в углу коробки он всё ещё
    // внутри секции, нажим остаётся единицей, и «тихие» кадры тихими
    // не будут.
    await page.mouse.move(4, 4);
    await page.waitForTimeout(600);
    const sh1 = await snyat(page, kor);
    await page.waitForTimeout(160);
    const sh2 = await snyat(page, kor);
    const shum = raznica(sh1, sh2);
    await page.mouse.move(cx, cy);
    await page.waitForTimeout(160);
    const o1 = await snyat(page, kor);
    const signal = raznica(sh2, o1);
    const otn = signal / Math.max(0.02, shum);
    const stroka = `картинка под курсором меняется в ${otn.toFixed(2)} раза сильнее `
      + `собственного дрейфа (сигнал ${signal.toFixed(2)}, дрейф ${shum.toFixed(2)})`;
    // Вердикта нет намеренно — см. OTKLIK_SPRAVKA наверху файла.
    console.log(`      —  ${stroka}; вердикта нет, мера не разделяет`);

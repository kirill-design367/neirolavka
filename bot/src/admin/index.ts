  ...
  return async function panel(req: IncomingMessage, res: ServerResponse): Promise<void> {
    ...
    if (req.method === 'GET') {
      ...
      if (put === `${KOREN}/statistika`) {
        /* СТАТИСТИКА ОТКРЫТА И ПОМОЩНИКУ — ЦЕЛИКОМ, ВКЛЮЧАЯ ВЫРУЧКУ.
           Решение владельца, сентябрь 2026, и прежнее «только
           владельцу» снято им же. Довод, по которому раздел был
           закрыт, — «чтобы отдать логин и пароль, знать выручку
           не нужно» — остался верным про НЕОБХОДИМОСТЬ, но раздел
           закрывали не поэтому: за ним нет ни одной двери к деньгам.
           Читать — не значит менять: страница только показывает
           числа, а всё, чем на этой же странице можно что-то
           СДЕЛАТЬ, по-прежнему спрашивает владельца (см. ниже
           про размеченные ссылки).

           Границу надо держать там, где она проведена: деньги
           двигают «Покупатели» и отметка оплаты, прайс — «Каталог»,
           скидки — «Промокоды», витрину — «Выкладка». Всё это
           осталось закрытым, и открытие статистики их не касается
           ни страницей, ни действием. */
        return otdat(
          res,
          200,
          str.statistika(

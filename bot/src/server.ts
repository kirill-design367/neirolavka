...
/**
 * Сколько живёт посчитанный счётчик подписок.
 *
 * Он спрашивается на каждой загрузке витрины, а не по действию
 * человека, — то есть весь трафик сайта приходит в процесс бота.
 * Сам `COUNT(*)` дёшев, но дешёвое, помноженное на посещаемость,
 * дешёвым быть перестаёт, а лавка стоит на двух ядрах вместе
 * с nginx.
 *
 * Минута — заведомо меньше, чем промежуток между двумя выдачами
 * в ручной лавке, и заведомо больше, чем всплеск посетителей.
 * Это же число уходит в `Cache-Control`: два срока годности
 * на одно значение разъехались бы.
 */
const SCHETCHIK_ZHIVET_MS = 60_000;

/**
 * Счётчик выданных заказов с памятью на минуту.
 *
 * Память живёт в процессе намеренно, а не в базе: терять её при
 * перезапуске не жалко — следующий же запрос пересчитает, — а вот
 * лишняя таблица ради кеша была бы вторым источником правды о числе,
 * которое и так считается одним запросом.
 */
let schetchikChislo = 0;
let schetchikDo = 0;

function schetchik(l: Lavka): number {
  const seychas = Date.now();
  if (seychas < schetchikDo) return schetchikChislo;
  schetchikChislo = zakazy.skolkoVydano(l.db);
  schetchikDo = seychas + SCHETCHIK_ZHIVET_MS;
  return schetchikChislo;
}

/** Забыть посчитанное. Нужно проверкам: у них своя база на каждый прогон. */
export function zabytSchetchik(): void {
  schetchikDo = 0;
}
...
export function sozdatServer(l: Lavka, vypusk: string, sostoyanie: Sostoyanie): Sluzhba {
  ...
  const server = createServer((req: IncomingMessage, res: ServerResponse) => {
     ...
     * в коде и не менялась никогда. Сайт статический, базы не видит —
     * значит живое число он может только СПРОСИТЬ, тем же способом,
     * каким спрашивает про промокод и про состояние заказа.
     *
     * НАРУЖУ УХОДИТ ОДНО ЧИСЛО — сколько заказов выдано. Прибавляет
     * к нему засев (2 417) сам сайт: там это число уже есть, лежит
     * в `catalog.subscribers` и служит запасным значением, когда бот
     * не ответил. Сложи мы здесь — засев пришлось бы держать в двух
     * местах, и второй экземпляр разъехался бы с первым молча.
     *
     * Почему считаются именно ВЫДАННЫЕ — в `zakazy.skolkoVydano`,
     * там же и оба отвергнутых варианта.
     *
     * ОТВЕТ КЕШИРУЕТСЯ, и это единственное отличие от двух соседних
     * путей. Те спрашивают по ДЕЙСТВИЮ человека — ввёл код, нажал
     * «Оплатить»; этот спрашивается НА КАЖДОЙ ЗАГРУЗКЕ СТРАНИЦЫ, то
     * есть весь трафик витрины приходит в процесс бота. Минута
     * задержки счётчику ничего не стоит: между выдачей заказа
     * и лишней единицей в шапке никто не сидит с секундомером.
     *
     * Своя зона предела частоты в nginx — тоже поэтому. Общая
     * с `/api/zakaz` означала бы, что посетители витрины съедают
     * бюджет у того, кто в эту минуту нажимает «Оплатить».
     */
    if (adres === '/api/schetchik') {
      const zagolovki = {
        'content-type': 'application/json; charset=utf-8',
        // Минуту держать можно: см. выше. `public` — чтобы значение
        // осело и у браузера, а не только в нашей памяти.
        'cache-control': `public, max-age=${Math.round(SCHETCHIK_ZHIVET_MS / 1000)}`,
        'x-content-type-options': 'nosniff',
      };
      if (req.method !== 'GET') {
        res.writeHead(405, zagolovki).end(JSON.stringify({ vydano: null }));
        return;
      }
      try {
        res.writeHead(200, zagolovki).end(JSON.stringify({ vydano: schetchik(l) }));
      } catch (e) {
        // Поломка базы — не повод отдать ноль: ноль на витрине
        // прочитался бы как «ещё никто ничего не купил». Сайт
        // на отказ оставляет засев, и это честнее любого числа.
        zhurnal.oshibka('счётчик подписок не сосчитался:', e);
        res.writeHead(503, zagolovki).end(JSON.stringify({ vydano: null }));
      }
      return;
    }
    /**
 ...
 */
/**
 * Страница, на которую человек возвращается из Робокассы.
 *
 * КНОПКА ОТКРЫВАЕТ БОТ В НОВОЙ ВКЛАДКЕ, И ЭТО НЕ КОСМЕТИКА.
 * Здесь, и только здесь, отдаётся ссылка с секретом заказа: уведи
 * она из этой же вкладки — и страница вместе с секретом пропадает.
 * Человек, у которого Telegram не открылся с первого раза (не тот
 * аккаунт, приложение не установлено, нажал «назад» не туда), терял
 * единственное, что у него было после оплаты. Копия ссылки лежит
 * в квитанции на сайте, но это ДРУГАЯ вкладка и другое устройство
 * может её не иметь: ниточек две, и рвать одну из них по своей воле
 * незачем.
 *
 * То же верно и для остальных четырёх видов этой страницы — там нет
 * секрета, но есть объяснение, что произошло с деньгами, и терять
 * его ради перехода тоже не за что.
 *
 * `rel` обязателен при `target="_blank"`: `noopener` не даёт открытой
 * странице дотянуться до `window.opener`, а `noreferrer` снимает
 * вопрос о заголовке Referer. Умолчание браузеров
 * (`strict-origin-when-cross-origin`) и так отдаёт наружу один домен,
 * без строки запроса с подписью возврата, — но это умолчание, а не
 * наше решение, и здесь в запросе лежит `SignatureValue`.
 *
 * Цена названа прямо: на телефоне `target="_blank"` открывает пустую
 * вкладку, которая тут же передаёт ссылку приложению Telegram, и эта
 * вкладка остаётся висеть. Это и есть плата за сохранённую страницу.
 */
...
<h1>${ekr(zagolovok)}</h1>
<p>${ekr(text)}</p>
<a href="${ekr(botUrl)}" target="_blank" rel="noopener noreferrer">${ekr(podpisKnopki)}</a>
</main></body></html>`;
}
  ...
  if (adres === '/robokassa/uspeh') {
    ...
    if (zakaz && zakaz.istochnik === 'sayt' && zakaz.klyuch && zakaz.tg_id === null) {
      html(
        200,
        'Оплата принята',
        `Спасибо, деньги получены. Ваш заказ: ${zakaz.nazvanie}. ` +
    ...
    if (zakaz && zakaz.istochnik === 'sayt') {
      html(
        200,
        'Оплата принята',
        `Спасибо, деньги получены. Ваш заказ «${zakaz.nazvanie}» уже у вас в боте — ` +
          'откройте «Мои заказы».',
      );
      return;
    }

...
`,
  ...
  {
    imya: '009-otkazy-uvedomleniy',
    sql: `
-- ОТКЛОНЁННЫЕ УВЕДОМЛЕНИЯ ОБ ОПЛАТЕ.
--
-- Уведомление, которое мы не взяли, — это деньги, которые пришли,
-- а заказ остался неоплаченным. Робокасса будет повторять его
-- раз в минуту, пока не получит «OK», и в журнале от этого одна
-- строка в минуту без единой подробности: разобраться по ней нельзя,
-- а повторить уведомление руками нечем — второй раз его не пришлют.
--
-- Поэтому отказ ЗАПИСЫВАЕТСЯ, и проба подписи разбирает его потом,
-- не тратя ни копейки и не дожидаясь новой оплаты.
--
-- В pary лежат ТОЛЬКО подписные поля (OutSum, InvId, SignatureValue,
-- IsTest, Shp_*). Робокасса кладёт в уведомление и почту плательщика,
-- а персональных данных лавка не собирает нигде; имена остальных
-- полей хранятся отдельно списком — имя поля персональными данными
-- не является, а для разбора подписи знать его нужно.
--
-- Пароля и ничего от него производного здесь нет и быть не может:
-- ожидаемую подпись проба считает сама, в своём процессе, из файла
-- окружения. База лежит с правами 700, пароли — в /etc под root
-- с правами 600, и переносить производное от них в место послабее
-- нельзя.
CREATE TABLE otkazy_uvedomleniy (
  id        INTEGER PRIMARY KEY AUTOINCREMENT,
  postavshchik TEXT NOT NULL,
  pochemu   TEXT    NOT NULL,
  -- Подписные поля как пришли, JSON.
  pary      TEXT    NOT NULL,
  -- Имена ВСЕХ пришедших полей, JSON-список. Только имена.
  imena     TEXT    NOT NULL,
  -- Сколько раз пришло то же самое. Робокасса повторяет ежеминутно,
  -- и двадцать одинаковых строк вместо одной со счётчиком скрыли бы
  -- предыдущий отказ — тот, который как раз и надо посмотреть.
  povtorov  INTEGER NOT NULL DEFAULT 1,
  vpervye   TEXT    NOT NULL,
  poslednee TEXT    NOT NULL
);

CREATE INDEX otkazy_svezhie ON otkazy_uvedomleniy(poslednee DESC);
`,
  },
];

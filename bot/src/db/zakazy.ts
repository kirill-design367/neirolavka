...
/**
 * Сколько заказов ВЫДАНО за всю жизнь лавки.
 *
 * Одно число наружу, для счётчика в шапке сайта. Считаются ровно
 * выданные, и выбор этот стоит объяснить — строка на сайте говорит
 * «оформили подписки», то есть обещает свершившийся факт.
 *
 * Почему не «оформленные». Заказ заводится нажатием «Оплатить»
 * на сайте, ещё до всякой оплаты, и заводится он ничейным: любой
 * человек с браузером накручивал бы счётчик нажатием кнопки, ничего
 * не заплатив. Число, которое можно надуть даром, — не факт,
 * а украшение, а раздел «Доверие в оформлении» прямо запрещает
 * выдуманные числа.
 *
 * Почему не «оплаченные». Оплаченный заказ ОТМЕНЯЕТСЯ — деньги
 * уходят на баланс, — и счётчик поехал бы ВНИЗ. Число на витрине,
 * которое иногда уменьшается, человек читает не как честность,
 * а как поломку.
 *
 * `vydan` свободен от обоих: его нельзя поставить самому себе
 * (ставит помощник, и только после доставки доступа) и нельзя снять
 * (`otmenit` принимает лишь `OTKRYTYE`, а `vydan` в них не входит).
 * Значит ряд неубывающий по построению, а не по договорённости.
 */
export function skolkoVydano(db: Baza): number {
  return (db.prepare("SELECT COUNT(*) n FROM zakazy WHERE status = 'vydan'").get() as { n: number }).n;
}

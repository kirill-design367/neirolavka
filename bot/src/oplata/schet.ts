...
export async function prinyatUvedomlenie(
  ...
  /* ДЕНЬГИ ПРИШЛИ ПО ЗАКРЫТОМУ ЗАКАЗУ. Так бывает: человек ушёл
     ...
     У НИЧЕЙНОГО ЗАКАЗА БАЛАНСА НЕТ, и придумать его некому: заказ
     оплачен на сайте, а входа на сайте нет. Положить такие деньги
     «куда-нибудь» значило бы отдать их чужому человеку. Поэтому
     единственное честное действие — не трогать их вовсе и позвать
     людей ГРОМКО: платёж записан, заказ виден в панели, разбирается
     это руками. Молчание здесь было бы потерянными деньгами. */
  if (zakaz.tg_id === null) {
    zakazy.sobytie(
      l.db,
      zakaz.id,
      'оплата пришла по закрытому ничейному заказу — деньги НЕ зачислены',
      null,
      `счёт № ${u.nomer}`,
    );
    zhurnal.oshibka(
      `оплата: ничейный заказ № ${zakaz.id} уже закрыт, ${u.summaKop} коп. никому не зачислены`,
    );
    void uved
      .komande(
        l,

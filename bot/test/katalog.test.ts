...
test('продукт без уровней читается с ПУСТЫМ списком, а не с выдуманным уровнем', () => {
  /* Не «один уровень», а НИ ОДНОГО: подписка одна, и это свойство
     ...
     ЗАВОДИМ ЕГО САМИ, А НЕ ИЩЕМ В ПРАЙСЕ. Здесь стояло сначала
     «у Seedance уровней нет», потом «в каталоге есть хоть один такой»,
     и оба раза проверка краснела на выкладке владельца: сперва
     у Seedance появились уровни, потом — у всех шести продуктов
     разом. Каталог при этом был исправен, путь покупки с карточки
     жив, а проверка падала не по той причине, ради которой написана.

     Прайс принадлежит владельцу, и он вправе завести уровни кому
     угодно. Проверять надо ЧТЕНИЕ такого продукта, а для этого его
     достаточно создать. Тот же закон, что у `ZHIVOY_PLAN`
     и `PRODUKT_BEZ_UROVNEY` в стенде. */
  const db = baza();
  bdKatalog.sozdatProdukt(db, 'proba-bez-urovney', 'Proba bez urovney');

  const bez = tovary(db).filter((t) => t.plans.length === 0);
  assert.ok(bez.length > 0, 'созданный продукт без уровней из каталога не читается');
  for (const t of bez) assert.equal(t.plans.length, 0, `${t.id}: уровни дорисовались`);

  // И у него НЕТ уровня с его же идентификатором: разбор кнопки
  // покупки различает продукт и уровень по составному id.
  assert.equal(tarif(db, 'proba-bez-urovney'), null, 'у продукта без уровней нашёлся уровень');
});
